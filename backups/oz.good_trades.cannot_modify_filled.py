...
class OZTrader(EWrapper, EClient):
    ...
    def orderStatus(
        ...
        self.remaining_by_order[orderId] = float(remaining)
    ...
    def tickByTickAllLast(
        ...
        if float(size) >= 100.0:
            ...
            # optional: debug
            print(f"New ref_price from trade >=100: {price} size={size}")
    ...
    def sync_orders(self):
        ...
        if buy_qty > 0:
            if self.buy_order_id is None:
                ...
                rem = self.remaining_by_order.get(self.buy_order_id)
                print("buy_qty = ", buy_qty, " Remaining buy qty = ", rem)
                if (rem == buy_qty):
                    print("No need to update buy order")
                else:
                    order = self.make_pegbest_order(
                        "BUY", buy_qty, buy_limit,
                        min_compete_size=min_compete_size,
        ...
        if sell_qty > 0:
            if self.sell_order_id is None:
                ...
                rem = self.remaining_by_order.get(self.sell_order_id)
                print("sell_qty = ", sell_qty, " Remaining sell qty = ", rem)
                if (rem == sell_qty):
                    print("No need to update sell order")
                else:
                    order = self.make_pegbest_order(
                        "SELL", sell_qty, sell_limit,
                        min_compete_size=min_compete_size,
