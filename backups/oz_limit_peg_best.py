...
MAX_POSITION = 200
# 0.10 no trades on 4/7/26
DELTA = 0.01  # buy at mid-delta, sell at mid+delta
TICK_SIZE = 0.01
...
class MidDeltaMarketMaker(EWrapper, EClient):
    ...
    def error(self, reqId, errorCode, errorString, advancedOrderRejectJson="", errorTime=""):
        benign_codes = {
            ...
            1775588109039,  # Market data farm connection is OK
            1775588109041,  # Market data farm connection is OK
            1775657275988,  # Market data farm connection is OK
        }

        if errorCode in benign_codes:
            return
    
        if errorString in benign_codes:
            return
        ...
        # if any(errorString.startswith(prefix) for prefix in benign_prefixes):
        #     return
    ...
    #        f"type={getattr(order, 'orderType', None)} lmt={getattr(order, 'lmtPrice', None)} "
    #        f"notHeld={getattr(order, 'notHeld', None)} status={orderState.status}"
    #    )
    ...
    def orderStatus(self, orderId, status, filled, remaining, avgFillPrice,
        ...
        if orderId == self.buy_order_id:
            ...
            if status in {"Inactive", "Cancelled", "ApiCancelled", "Filled"}:
                if status == "Inactive" or remaining == 0:
                    print(f"RESET BUY ORDER ID due to status={status}")
                    self.buy_order_id = None
                    self.buy_live = False
            ...
            if status in {"Inactive", "Cancelled", "ApiCancelled", "Filled"}:
                if status == "Inactive" or remaining == 0:
                    print(f"RESET SELL ORDER ID due to status={status}")
                    self.sell_order_id = None
                    self.sell_live = False
    ...
    def make_peg_best_order(self, action: str, qty: int, limit_price: float) -> Order:
        o = Order()
        o.action = action
        o.orderType = "PEG BEST"
        o.totalQuantity = qty
        o.lmtPrice = limit_price  # protective cap/floor
        o.notHeld = True
        o.tif = "DAY"
        o.outsideRth = False
        o.transmit = True
    ...
    def place_or_modify_buy(self, contract):
        ...
        if self.buy_order_id is None:
            ...
            order = self.make_peg_best_order("BUY", qty, desired)
            self.placeOrder(self.buy_order_id, contract, order)
            self.buy_live = True
            self.last_buy_lmt = desired
            self.last_buy_qty = qty
            print(f"PLACED BUY PEG BEST qty={qty} cap={desired:.2f}")
            return
        ...
        if need_modify:
            ...
            self.last_buy_lmt = desired
            self.last_buy_qty = qty
            print(f"MODIFIED BUY PEG BEST qty={qty} cap={desired:.2f}")
    ...
    def place_or_modify_sell(self, contract):
        ...
        if self.sell_order_id is None:
            ...
            order = self.make_peg_best_order("SELL", qty, desired)
            self.placeOrder(self.sell_order_id, contract, order)
            self.sell_live = True
            self.last_sell_lmt = desired
            self.last_sell_qty = qty
            print(f"PLACED SELL PEG BEST qty={qty} floor={desired:.2f}")
            return
        ...
        if need_modify:
            ...
            self.last_sell_lmt = desired
            self.last_sell_qty = qty
            print(f"MODIFIED SELL PEG BEST qty={qty} floor={desired:.2f}")
...
def main():
    ...
    app.reqMarketDataType(3)  # delayed streaming if live is unavailable
    time.sleep(0.25)
    ...
    while app.current_mid() is None:
        if time.time() - wait_start > 15:
            print(f"Still waiting... bid={app.bid} ask={app.ask}")
            wait_start = time.time()
        time.sleep(0.25)
    ...
    try:
        while True:
            ...
            if mid is not None:
                ...
                if mid != previous_mid or app.current_position != previous_position:
                    previous_mid = mid
                    previous_position = app.current_position
                    print(
                        f"bid={app.bid:.2f} ask={app.ask:.2f} mid={mid:.4f} pos={app.current_position} "
                        f"buy_qty={buy_qty} buy_cap={buy_px:.2f} buy_cd={max(0, app.buy_cooldown_until-time.time()):.2f}s "
                        f"sell_qty={sell_qty} sell_floor={sell_px:.2f} sell_cd={max(0, app.sell_cooldown_until-time.time()):.2f}s"
                    )
