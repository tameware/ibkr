...
class OZTrader(EWrapper, EClient):
    def __init__(self):
        ...
        self.ref_price = None      # midprice reference
    ...
    def orderStatus(
        ...
        if status in ("Filled", "Cancelled"):
            if orderId == self.buy_order_id:
                print(f"BUY {orderId} done ({status}); clearing pending_buy")
                self.pending_buy = False
                self.buy_order_id = None
            if orderId == self.sell_order_id:
                print(f"SELL {orderId} done ({status}); clearing pending_sell")
                self.pending_sell = False
                self.sell_order_id = None
    ...
    def tickByTickAllLast(
        ...
        if float(size) >= 1:
            ...
            print(f"New ref_price from trade >=1: {price} size={size}")
    ...
    def tickPrice(self, reqId: TickerId, tickType: TickType, price: float, attrib: TickAttrib):
        ...
        if self._bid is not None and self._ask is not None and self._ask > 0:
            ...
            self.ref_price = mid
            print(f"Midprice updated: bid={self._bid} ask={self._ask} mid={mid}")
    ...
    def openOrderEnd(self):
        ...
        print(f"Open OZ orders: buys={self.open_oz_buys}, sells={self.open_oz_sells}")
    ...
    def historicalDataEnd(self, reqId, start, end):
        ...
        if last_bar.open and last_bar.open > 0:
            ...
            print(f"Today's open price: {self.open_price}")
        else:
    ...
    def sync_orders(self):
        ...
        print("sync_orders: self.ref_price = ", self.ref_price)
        if self.ref_price is None:
            return
        ...
        ref = self.ref_price
        buy_limit = round(ref * 1.02)
        sell_limit = round(ref * 0.98, 2)
